...
import com.whatsnumber.isysoi.whatsthenumber.R.id.*
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var appLaunchValue  = sharedPref.getInt("APP_LAUNCH_COUNTER_ID", 0)
        if (appLaunchValue == 0) {
            editor.putInt("APP_ID", randomInt(0,123241))
        }
        editor.putInt("APP_LAUNCH_COUNTER_ID", ++appLaunchValue)
        editor.apply()
        val appID = sharedPref.getInt("APP_ID", 0)

        appInfoTextView.text = """${appID} : ${appLaunchValue}"""
